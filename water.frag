...
void main(void) 
{
	...
	*/
	...
	vec3 specularHighlight = lightColour * specular * reflectivity;

    //out_Color = mix(reflectColor, refractColor, 0.5);
	out_Color = mix(reflectColor, refractColor, refractiveFactor);
	out_Color = mix(out_Color, vec4(0, 0.3, 0.5, 1), 0.2) + vec4(specularHighlight, 0);
	//out_Color = normalMapColour;
