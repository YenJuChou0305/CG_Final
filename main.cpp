...
void renderScene(Shader wallShader, unsigned int wallVAO, float P[4]);
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		camera.Pitch = -camera.Pitch;		//camera.invertPitch();
...
void renderScene(Shader wallShader, unsigned int wallVAO, float P[4])
{
	...
	glUniform4fv(plane_location, 1, P);
